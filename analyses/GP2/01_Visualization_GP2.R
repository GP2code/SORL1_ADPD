...
load_block_file <- function(ancestry) {
  file_path <- file.path(block_folder, paste0(ancestry, "_cases_phased_blocks_imputed.blocks.det"))
...
plot <- ggplot(all_blocks, aes(x = BP1, xend = BP2, y = Ancestry, yend = Ancestry)) +
  ...
  theme(
    plot.title = element_text(size = 6.6),
...
# Save plot
ggsave("~/Desktop/haplotype_blocks_colored_cases_imputed_variants.png",
